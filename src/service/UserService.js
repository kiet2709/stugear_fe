...
const USER_URL = "http://52.205.41.43/api/users";
...
class UserService {
  ...
  removeCurrentUserWishListByProductId(id) {
    console.log(id);
    return axiosPrivate
      .post("http://52.205.41.43/api/wishlists/remove", {
        product_id: id,
      })
      .then((response) => response?.data?.data)
      .catch((error) => error?.response);
  ...
  getCurrentUserProducts(currentPage) {
    let url = `http://52.205.41.43/api/products/current`;
  ...
  sendVerifyEmail(email) {
    return axiosPrivate
      .get(
        `http://52.205.41.43/api/products/send-verify-email?email=${email}`
      )
      .then((response) => response?.data)
      .catch((error) => error?.response);
  }
